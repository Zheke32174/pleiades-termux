...
def root_path() -> pathlib.Path:
    return pathlib.Path(os.environ.get("PLEIADES_ROOT", pathlib.Path.home() / ".local" / "share" / "pleiades-edge")).expanduser()
...
def next_sequence() -> int:
    ...
    sequence_path = layout["state"] / "sequence"
    initialize()
    with sequence_path.open("r+", encoding="ascii") as handle:
...
def append_event(event: dict[str, Any]) -> None:
    queue_path = paths()["queue"] / "events.jsonl"
    ensure_private_directory(queue_path.parent)
...
def emit(event_type: str, severity: str, source: str, payload: dict[str, Any]) -> dict[str, Any]:
    node = initialize()
...
def read_events(limit: int | None = None) -> list[dict[str, Any]]:
    ...
    if not queue_path.exists():
        return []
    lines = queue_path.read_text(encoding="utf-8", errors="strict").splitlines()
    ...
    for number, line in enumerate(lines, start=1):
        ...
        try:
            ...
            raise EdgeError(f"queue contains invalid JSON at selected line {number}") from exc
        if not isinstance(value, dict) or value.get("schema") != EVENT_SCHEMA:
            raise EdgeError(f"queue contains an invalid event at selected line {number}")
        events.append(value)
    return events
...
def export_queue(destination: pathlib.Path) -> pathlib.Path:
    initialize()
    source = paths()["queue"] / "events.jsonl"
...
def doctor() -> tuple[dict[str, Any], bool]:
    ...
    checks = {
        ...
        "node_private": permission_string(layout["config"] / "node.json") in {"0o600", "0o400"},
...
def parse_payload(raw: str | None, path: pathlib.Path | None) -> dict[str, Any]:
    if raw and path:
...
def main() -> int:
    ...
    try:
        if args.command == "init":
            ...
            initialize()
            print(json.dumps(load_json(paths()["config"] / "capabilities.json"), indent=2, sort_keys=True))
            ...
            print(json.dumps(emit(args.event_type, args.severity, args.source, payload), indent=2, sort_keys=True))
